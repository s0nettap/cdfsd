...
class c_rage_bot {
	...
	uint32_t get_random_seed( vec3_t view_angles, int player_tick ) {
		using get_random_seed_t = __int64( __fastcall* )( int, vec3_t*, int );
	...
	vec3_t get_removed_aim_punch_angle( c_cs_player_pawn* local_player ) {
		...
		static fnUpdateAnimPunch fn = reinterpret_cast<fnUpdateAnimPunch>(
			g_opcodes->scan_absolute( g_modules->m_modules.client_dll.get_name( ), xorstr_( "E8 ? ? ? ? 48 8D 4E ? 48 8D 54 24 ? E8 ? ? ? ? F2 0F 10 44 24" ), 0x1 ) );
