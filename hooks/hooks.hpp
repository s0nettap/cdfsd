...
class c_hook {
    void* m_function;
    void* m_detour;
    LPVOID* m_trampoline; // Изменён тип на LPVOID* для совместимости с MinHook
public:
    c_hook() : m_function(nullptr), m_detour(nullptr), m_trampoline(nullptr) {}
    void hook(void* target, void* dtr) {
        m_function = target;
        m_detour = dtr;
        uintptr_t obfuscated = reinterpret_cast<uintptr_t>(target) ^ (rand() % 0x1000); // Оbfuscation with random XOR
        MH_CreateHook(reinterpret_cast<void*>(obfuscated), m_detour, m_trampoline);
        MH_EnableHook(reinterpret_cast<void*>(obfuscated));
    }
    ...
    void unhook() {
        if (!m_function || !m_detour || !m_trampoline)
            return;
        MH_DisableHook(m_function);
        MH_RemoveHook(m_function);
        m_function = nullptr;
        m_detour = nullptr;
        m_trampoline = nullptr; // Clear traces
    }
};
...
namespace hooks
{
    ...
    // Random delay for anti-VAC detection
    inline void random_delay() {
        std::this_thread::sleep_for(std::chrono::microseconds(rand() % 50));
    }

    // Smooth angle correction to avoid VAC triggers
    inline void smooth_angle(vec3_t& angle, const vec3_t& target, float smoothing = 0.1f) {
        angle.x = angle.x + (target.x - angle.x) * smoothing;
        angle.y = angle.y + (target.y - angle.y) * smoothing;
        angle.z = angle.z + (target.z - angle.z) * smoothing;
